---
type: synthesis
title: Geno Auriemma Team Leadership Framework
created: 2026-04-08
updated: 2026-04-08
status: active
tags:
  - synthesis
  - leadership
  - teams
question: What durable team-leadership framework can be extracted from the Geno Auriemma masterclass summary?
basis:
  - wiki/sources/2026-04-08-geno-auriemma-leadership-and-team-building-masterclass-complete-summary.md
  - wiki/concepts/leadership/leadership/high-performance-team-culture.md
  - wiki/concepts/leadership/leadership.md
related:
  - wiki/entities/geno-auriemma.md
  - wiki/syntheses/cross-source/high-performance-leadership-models.md
---

# Question

What durable team-leadership framework can be extracted from the Geno Auriemma masterclass summary?

# Answer

The current source base supports a six-part framework:

1. Put the team above the individual ego.
   The standard is collective excellence, not isolated talent.

2. Lead as service.
   Take hits, provide tools, and stay available rather than performing authority.

3. Build trust aggressively.
   Do not lie, do not give false hope, and do not break commitments.

4. Make accountability visible.
   Shared standards and direct correction reduce gossip and confusion.

5. Train above game conditions.
   Practice pressure should exceed performance pressure.

6. Distinguish fair from equal.
   Common standards can still require situational judgment.

# Basis

- Main source: [[wiki/sources/2026-04-08-geno-auriemma-leadership-and-team-building-masterclass-complete-summary|Geno Auriemma Leadership and Team Building Masterclass Complete Summary]]
- Canonical concepts: [[wiki/concepts/leadership/leadership/high-performance-team-culture|High-Performance Team Culture]], [[wiki/concepts/leadership/leadership|Leadership]]

# Gaps

- The branch still rests on a secondary summary and on elite sports experience.
- The vault needs later sources that challenge public accountability and high-pressure training from more psychologically protective team models.

# Related Pages

- [[wiki/entities/geno-auriemma|Geno Auriemma]]
- [[wiki/concepts/leadership/leadership/high-performance-team-culture|High-Performance Team Culture]]
- [[wiki/concepts/leadership/leadership|Leadership]]
- [[wiki/syntheses/cross-source/high-performance-leadership-models|High-Performance Leadership Models]]

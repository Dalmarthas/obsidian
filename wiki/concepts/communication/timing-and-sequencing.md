---
type: concept
title: Timing and Sequencing
created: 2026-04-08
updated: 2026-04-08
status: active
tags:
  - concept
  - timing
  - decision-making
  - persuasion
source_pages:
  - wiki/sources/2026-04-08-daniel-pink-persuasion-masterclass-complete-summary.md
related:
  - wiki/concepts/communication/persuasion.md
  - wiki/concepts/leadership/leadership/high-performance-mindset.md
  - wiki/syntheses/communication/daniel-pink-persuasion-framework.md
---

# What It Is

Timing and sequencing is the claim that outcomes depend not only on content but also on when a message or action appears, what comes before it, and how the experience ends. In the current source base, timing is not a minor optimization. It is part of the persuasive mechanism itself.

# Why It Matters

- It explains why good ideas still fail when delivered at the wrong moment or in the wrong order.
- It helps the vault treat influence as environmental design rather than pure rhetoric.
- It creates a bridge between persuasion, motivation, and project execution.

# Evidence In This Vault

- Pink argues that "when" matters alongside "what" and "how."
- Beginnings benefit from fresh-start effects and premortems that expose failure modes early.
- Midpoints can drag performance down or wake it up, depending on whether they are treated as a slump or an alarm.
- Endings have outsized force because they energize action and disproportionately shape memory.
- Pink also gives smaller sequence rules:
  - bad news before good news
  - visible short deadlines can outperform generous vague ones
  - early day and post-break asks can help overcome default no's

# Tensions / Open Questions

- Timing can improve the odds, but it does not rescue a weak offer or bad diagnosis.
- Sequence awareness can become manipulative if used only to steer emotion without improving the substance underneath.

# Related Pages

- [[wiki/concepts/communication/persuasion|Persuasion]]
- [[wiki/concepts/leadership/leadership/high-performance-mindset|High-Performance Mindset]]
- [[wiki/syntheses/communication/daniel-pink-persuasion-framework|Daniel Pink Persuasion Framework]]

